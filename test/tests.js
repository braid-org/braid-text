...
runTest(
    "test backup_init creates backups",
    async () => {
        var key = 'test-backup-' + Math.random().toString(36).slice(2)

        // First create some data
        var r = await braid_fetch(`/${key}`, {
            method: 'PUT',
            body: 'backup test data'
        })
        if (!r.ok) return 'put failed: ' + r.status

        // Now test the backup functionality using braid_text's own backup_init
        var r = await braid_fetch(`/eval`, {
            method: 'PUT',
            body: `void (async () => {
                var fs = require('fs')
                ...
                // Configure backups with a test folder and very short interval
                ...
                // Reset backup_init so it can run again
                braid_text.backup_init.started = false
                ...
                // Find the backup file for our key
                var files = await fs.promises.readdir(braid_text.db_folder)
                var our_file = files.find(f => f.includes('${key}') && /\\.\\d+$/.test(f))
                if (!our_file) {
                    res.end('file not found in db_folder')
                    return
                }

                var base_name = our_file.match(/^(.+)\\.\\d+$/)[1]
                var file_backup_dir = path.join(backup_folder, base_name)
                var backup_path = path.join(file_backup_dir, date)

                // Verify backup exists and contents match source
                try {
                    var src_path = path.join(braid_text.db_folder, our_file)
                    var src_contents = await fs.promises.readFile(src_path)
                    var backup_contents = await fs.promises.readFile(backup_path)

                    if (!src_contents.equals(backup_contents)) {
